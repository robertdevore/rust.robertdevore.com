...
Inter is self-hosted for paragraphs and general reading text, using the same licensed 400/700 assets as the Python course. Departure Mono remains self-hosted for headings, navigation, labels, controls, and code. The regular Inter and Departure Mono fonts are preloaded. Theme tokens retain the paper/rust palette. Scrollbars use matching light and dark track/thumb colors, with standard and WebKit rules and a forced-colors fallback.
