{"title":"Cargo, modules, and dependency boundaries","stage":2,"minutes":55,"summary":"Read the actual workspace, feature graph, lockfile, and release configuration.","example":"13_cargo"}
---
...
The workspace explicitly selects resolver `3`, appropriate for this edition-2024 course. Virtual workspaces should declare their resolver because there may be no root package edition from which to infer one. Resolver behavior and minimum-supported-Rust-version policy are related, but a declaration is not a substitute for testing your supported compiler.
...
Build scripts and procedural macros execute on the build machine. Review dependencies as executable code, including their transitive graph and maintenance status. A small handwritten error enum is appropriate here; Serde becomes useful when a real structured format is introduced. Do not add dependencies solely because a professional project is expected to have them.
...
The CLI graph contains the CLI and local core. Tokio belongs to the example package's graph. Workspace membership does not automatically make every member a dependency of every other member. A shared lockfile may list packages a particular target does not use.
