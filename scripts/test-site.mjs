...
const webmcp=JSON.parse(await readFile('dist/.well-known/kujo-site-index.json'));assert.equal(webmcp.schema,'kujo-ssg-site-index/v1');assert.equal(webmcp.site.url,origin);assert.equal(webmcp.items.filter(x=>x.type==='lessons').length,28);assert.ok((await stat('dist/assets/js/kujo-webmcp.js')).size<12288);assert.match(await readFile('dist/index.html','utf8'),/data-kujo-webmcp/);assert.doesNotMatch(await readFile('dist/404.html','utf8'),/data-kujo-webmcp/);
