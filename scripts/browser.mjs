...
const checked=[];
try{
for(const route of ['/', '/course/', '/practice/', '/about/',...course.map(l=>l.url)]){const r=await page.goto(base+route);assert.equal(r.status(),200,route);await page.locator('h1').waitFor();assert.equal(await page.locator('link[rel=canonical]').getAttribute('href'),'https://rust.robertdevore.com'+route);assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),`horizontal overflow ${route}`);checked.push(route);}
...
for(const l of course){await page.goto(base+l.url);assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),`mobile overflow ${l.url}`);}
