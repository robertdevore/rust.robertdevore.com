...
const browser=await chromium.launch(process.env.BROWSER_CHANNEL ? {channel:process.env.BROWSER_CHANNEL} : {});const context=await browser.newContext({viewport:{width:1440,height:1000}});const page=await context.newPage();const errors=[];const failed=[];const cancelledAnalytics=[];
page.on('pageerror',e=>errors.push(e.message));page.on('console',m=>{if(m.type()==='error')errors.push(m.text());});page.on('requestfailed',r=>{
 const url=new URL(r.url());const failure={url:url.origin+url.pathname,error:r.failure()?.errorText};
 // Navigation cancels background analytics. Preserve these separately; CSP,
 // console errors, asset failures and all other network failures still fail QA.
 if(failure.error==='net::ERR_ABORTED' && ['https://stats.g.doubleclick.net/g/collect','https://www.google.com/ads/ga-audiences'].includes(failure.url))cancelledAnalytics.push(failure);
...
await writeFile('work/qa/browser.json',JSON.stringify({base,checked,accessibility,errors,failed,cancelledAnalytics,checkedAt:new Date().toISOString()},null,2));
